...
async function addUser(obj) {
  try {
    await User.create({
      nick_name: obj.nick_name, email: obj.email, role: obj.role, checked: obj.checked, password: obj.password, home_id: obj.home_id,
    });
  } catch (err) {
    console.log(err);
  }
...
//   addUser(el);
// });
const chat = [{ user_id: 1, text: 'blabla' }, { user_id: 2, text: 'some words' }];
...
const benifit = [{
  ...
  user_id: 2, text: 'second benifit', title: 'second', price: 1000,
}];
...
const response = [{
  ...
  user_id: 1, local_news_id: 2, status: 'true',
}];
...
const like = [{
  ...
  user_id: 1, local_news_id: 2, count: 30,
}];
